...
void update_accumulator_full(Color                     perspective,
                             const FeatureTransformer& featureTransformer,
                             const Position&           pos,
                             AccumulatorState&         accumulatorState);

void update_accumulator_cache(Color                     perspective,
                              const FeatureTransformer& featureTransformer,
                              const Position&           pos,
...
void AccumulatorStack::evaluate(const Position&           pos,
    ...
    {
        evaluate_side(WHITE, pos, featureTransformer, last_white);
        evaluate_side(BLACK, pos, featureTransformer, last_black);
    }

    update_accumulator_cache(WHITE, featureTransformer, pos, cache);
    update_accumulator_cache(BLACK, featureTransformer, pos, cache);
}
...
void AccumulatorStack::evaluate_side(Color                     perspective,
    ...
    {
        update_accumulator_full(perspective, featureTransformer, pos, mut_latest());
...
usize AccumulatorStack::find_last_usable_accumulator(Color perspective) const noexcept {
    ...
    for (usize curr_idx = size - 1; curr_idx > 0; curr_idx--)
    {
        ...
        const auto& dirtyPiece = accumulators[curr_idx].dirtyPiece;
...
void apply_threat_changes(Color                              perspective,
                          const FeatureTransformer&          featureTransformer,
    ...
    for (IndexType j = 0; j < Dimensions / Tiling::TileHeight; ++j)
    {
        ...
        apply_threat_features<-1>(j, acc, removed, featureTransformer);
        apply_threat_features<+1>(j, acc, added, featureTransformer);
    ...
    for (IndexType j = 0; j < PSQTBuckets / Tiling::PsqtTileHeight; ++j)
    {
        ...
        apply_psqt<-1>(j, psqt, removed, featureTransformer.threatAndPpPsqtWeights.data());
        apply_psqt<+1>(j, psqt, added, featureTransformer.threatAndPpPsqtWeights.data());
...
void update_accumulator_incremental(Color                     perspective,
    ...
    apply_threat_changes(perspective, featureTransformer, computed, target_state, thrAdded,
                         thrRemoved);
...
void update_accumulator_incremental_both(const FeatureTransformer& featureTransformer,
    ...
    PairFeatureSet::append_changed_indices_both(
      ...
      thr_removed[BLACK], thr_added[BLACK], threat_pp_base, pf_stride);
    apply_threat_changes(WHITE, featureTransformer, computed, target_state, thr_added[WHITE],
                         thr_removed[WHITE]);
    apply_threat_changes(BLACK, featureTransformer, computed, target_state, thr_added[BLACK],
                         thr_removed[BLACK]);
...
// Rebuild the threat and pawn-pair accumulator from the active features.
void update_accumulator_full(Color                     perspective,
                             const FeatureTransformer& featureTransformer,
                             const Position&           pos,
    ...
    while (tileOffset < Dimensions)
    {
        ...
        auto        acc = __riscv_vmv_v_x_i16m8(0, vl);
        for (int i : active)
        {
            acc = __riscv_vwadd_wv_i16m8(
              acc, __riscv_vle8_v_i8m4(&threatWeights[i * Dimensions + tileOffset], vl), vl);
        }
        __riscv_vse16_v_i16m8(&accumulator.accumulation[perspective][tileOffset], acc, vl);
        tileOffset += vl;
    }
    ...
    while (tileOffset < PSQTBuckets)
    {
        ...
        auto        acc = __riscv_vmv_v_x_i32m1(0, vl);
        for (int i : active)
        {
            acc = __riscv_vadd_vv_i32m1(
              acc, __riscv_vle32_v_i32m1(&threatPsqtWeights[i * PSQTBuckets + tileOffset], vl), vl);
        }
        __riscv_vse32_v_i32m1(&accumulator.psqtAccumulation[perspective][tileOffset], acc, vl);
        tileOffset += vl;
    }
...
// Keep the HalfKA Finny entry for the current king square synchronized with the position.
void update_accumulator_cache(Color                     perspective,
                              const FeatureTransformer& featureTransformer,
                              const Position&           pos,
                              AccumulatorCaches&        cache) {
