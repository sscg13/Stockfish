...
inline IndexType Full_Threats::make_psq_index(Square s, Piece pc, Square ksq) {
    return IndexType(79856 + (int(s) ^ OrientTBLA[Perspective][ksq]) + PieceSquareIndex[Perspective][pc]
...
void Full_Threats::append_active_psq(const Position& pos, IndexList& active) {
    ...
    {
        Square s = pop_lsb(bb);
        active.push_back(make_psq_index<Perspective>(s, pos.piece_on(s), ksq));
    }
}
    ...
    for (int i = WHITE; i <= BLACK; i++) {
        for (int j = PAWN; j <= KING; j++) {
            ...
            if (pt == PAWN) {
                ...
                while (bb) {
                    Square from = pop_lsb(bb);
                    psq.push_back(make_psq_index<Perspective>(from, attkr, ksq));
                }
            }
            else {
                while (bb)
...
/*bool Full_Threats::requires_refresh(const StateInfo* st, Color perspective) {
    ...
    && OrientTBLT[perspective][st->dirtyPiece.from[0]] != OrientTBLT[perspective][st->dirtyPiece.to[0]]);
}*/
